...
const create = async (job) => {
    return new Promise(async (resolve, reject) => {
        try {
            const { name, id_type_job, salary_min, salary_max, quantity, description, request, id_language, id_location } = job.job;
            ...
            if (data[1]) {
                ...
                const dataLast = await db.post.findOrCreate({
                    ...
                    defaults: { id_business: job.id_business, status: 0, expire: date, step1: 0, step2: 0, step3: 0, step4: 0, step5: 0 },
                });
